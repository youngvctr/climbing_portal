...
const mySQLStore = require('express-mysql-session')(session) // 로그인, 인증처리 모듈 => mySQLStore에 저장
...
const { options, connection } = require('../src/db')
...
const sessionStore = new mySQLStore(options) // mysql 세션 저장
...
app.use(bodyParser.urlencoded({ extended: false })) // URL 인코딩된 본문만 구문 분석(parsing : 원하는 형태로 가공)
